...
async function main() {
  ...
  try {
    ...
    for (const theme of themes) {
      ...
      await page.goto(`${BASE_URL}/preview/${theme}`, {
      ...
      const themeScreenshot = await page.screenshot({
        type: "png",
        clip: { x: 0, y: 0, width: 1200, height: 1300 },
      });

      const themeFile = path.join(OUTPUT_DIR, `theme-${theme}.png`);
      await fs.writeFile(themeFile, themeScreenshot);
      console.log(`   ✅ Gespeichert: docs/images/theme-${theme}.png`);
    }

    // -------------------------------------------------------------
    // 2. Hero Preview Screenshot (Hauptseite)
    // -------------------------------------------------------------
    console.log(`📸 Erstelle Hero-Preview (/)...`);
    await page.setViewport({
      width: 1280,
      height: 1100,
      deviceScaleFactor: 2,
    ...
    const heroFile = path.join(OUTPUT_DIR, "hero-preview.png");
    await fs.writeFile(heroFile, heroScreenshot);
    console.log(`   ✅ Gespeichert: docs/images/hero-preview.png`);

    // -------------------------------------------------------------
    // 3. PDF Screenshots & Vergleichsbild (A4-Format)
    // -------------------------------------------------------------
    const A4_WIDTH = 794;
    ...
    console.log("📸 Erstelle ATS-PDF Screenshot (/print/ats)...");
    ...
    console.log(`   ✅ Gespeichert: docs/images/pdf-ats.png`);

    console.log("📸 Erstelle Design-PDF Screenshot (/print/design)...");
